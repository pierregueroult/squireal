<div class="flex flex-col gap-2 w-full">
  <div class="w-full h-14 border-2 border-text rounded-xl bg-foreground flex overflow-hidden" id="container-<?= $id ?>">
    <div class="flex-grow pl-4">
      <input type="<?= $type ?>" class="h-full w-full outline-none" id="input-<?= $id ?>" <?php if ($regex) : ?>data-valid="false"<?php endif; ?>
        placeholder="<?= lang($placeholder) ?>" required />
    </div>
    <div class="h-14 w-14 flex items-center justify-center border-s-2 border-text pb-1 <?= $regex ? 'bg-red-300' : 'bg-blue-300' ?>"
      id="validation-<?= $id ?>">
      <?php if ($regex) : ?>
