<header
  ...
  <button class="md:hidden flex items-center justify-center h-12 w-12 rounded-md bg-foreground text-text border-text"
    id="mobile-navigation-button">
    <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor"
      stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="absolute transition-transform">
    ...
    <ul class="flex items-center">
      ...
      foreach ($items as $item) {
        ...
        <li class="py-2 px-2 lg:px-4">
          <a href="<?= base_url() . $locale . $item["url"] ?>" class="font-sans text-md lg:text-xl <?= $isActive
  ...
  <nav class="flex flex-col p-8 gap-4">
    <ul class="flex flex-col gap-4">
      ...
      foreach ($items as $item) {
        $isActive = "/" . service("request")->uri->getSegment(2) === $item["url"]; ?>
        <li class="py-2 px-2 lg:px-4">
          <a href="<?= base_url() . $locale . $item["url"] ?>" class="font-main text-xl <?= $isActive
                  ? "underline underline-offset-4"
                  : "" ?>">
            <?= lang($item["name"]) ?>
          </a>
